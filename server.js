...
app.get('/users', (req, res) => {
    Firebird.attach(options, (err, db) => {
        ...
        db.query(`SELECT USU_COD, USU_NOME FROM USUARIOS WHERE USU_ATIVO='S'`, [], (err, result) => {
            db.detach();
            res.json(result ? result.map(u => ({ id: u.USU_COD.toString(), name: safeString(u.USU_NOME), canTreat: false })) : []);
        });
    });
});
...
app.get('/categories', (req, res) => {
    Firebird.attach(options, (err, db) => {
        ...
        db.query('SELECT GR_COD, GR_DESCRI FROM GRUPOPRODUTOS', [], (err, groups) => {
            ...
            db.query('SELECT GR_COD, SG_COD, SG_DESCRI FROM SUBGRUPOPRODUTOS', [], (err, subs) => {
                db.detach();
                const tree = groups.map(g => ({
                    ...
                    count: 0,
                    subcategories: subs.filter(s => s.GR_COD === g.GR_COD).map(s => ({
                        id: String(s.SG_COD), db_id: s.SG_COD, name: safeString(s.SG_DESCRI), count: 0, icon: 'circle'
                    }))
                }));
                res.json(tree);
            });
...
// --- NOVA ROTA 1: SUGESTÕES INTELIGENTES (PEDIDOS + CICLO) ---
app.get('/daily-meta-suggestions', (req, res) => {
    ...
    Firebird.attach(options, async (err, db) => {
        ...
        try {
            // 1. Calcular Meta Efetiva (Base + Atrasado)
            let effectiveTarget = dailyTarget;
            if (accumulationMode) {
                // Soma saldo devedor dos últimos 3 dias (simplificado)
                const sqlLog = `SELECT COUNT(*) AS TOTAL FROM GRIDE_INVENTARIO_LOG WHERE DATA_HORA >= DATEADD(-3 DAY TO CAST('NOW' AS DATE)) AND DATA_HORA < CAST('NOW' AS DATE)`;
                ...
                effectiveTarget += Math.min(deficit, Math.floor(dailyTarget * 0.5)); // Cap 50% extra
            }

            // 2. IDs Bloqueados (Reservados ou Tratamento)
            ...
            // 3. Fila de GIRO (Baseada em PEDIDOSITENS)
            // Itens ativos, vendidos nos ultimos 60 dias, sem contagem recente
            const giroLimit = Math.floor(effectiveTarget * 0.5); // 50% da meta para giro
            
            let giroIds = [];
            try {
                const sqlGiro = `
                    SELECT FIRST ${giroLimit} P.PRO_COD
                    FROM PEDIDOSITENS PI
                    JOIN PRODUTOS P ON P.PRO_COD = PI.PRO_COD
                    WHERE P.PRO_ATIVO = 'S'
                    AND PI.DATA >= DATEADD(-60 DAY TO CURRENT_DATE)
                    ${notInClause}
                    AND (P.PRO_ULTIMA_CONTAGEM IS NULL OR P.PRO_ULTIMA_CONTAGEM < DATEADD(-${cooldownDays} DAY TO CURRENT_DATE))
                    GROUP BY P.PRO_COD
                ...
                console.log("Erro na query de Giro (tabela PEDIDOSITENS existe?):", e.message);
            }

            // 4. Fila de CICLO (Antiguidade)
            // Preenche o resto da meta
            const cycleLimit = effectiveTarget - giroIds.length;
            const ignoreIds = [...lockedIds.split(','), ...giroIds].filter(x => x).join(',');
...
// --- NOVA ROTA 2: STATUS DA META (ACUMULADO) ---
app.get('/daily-meta-status', (req, res) => {
    ...
    Firebird.attach(options, async (err, db) => {
        ...
        try {
            ...
            if (accumulate) {
                // Deficit ultimos 3 dias
                const sqlPast = `SELECT COUNT(*) AS TOTAL FROM GRIDE_INVENTARIO_LOG WHERE DATA_HORA >= DATEADD(-3 DAY TO CAST('NOW' AS DATE)) AND DATA_HORA < CAST('NOW' AS DATE)`;
                ...
                accumulatedPending = Math.min(accumulatedPending, Math.floor(target * 0.5)); // Cap
            }
...
app.get('/blocks', (req, res) => {
    // Rota legada para 'Explorar Estoque' (busca geral)
        ...
        db.transaction(Firebird.ISOLATION_READ_COMMITTED, async (err, transaction) => {
            try {
                ...
                for (const item of items) {
                    ...
                    if (item.status === 'not_located' || item.status === 'divergence_info') {
                        ...
                        await new Promise((res, rej) => transaction.query(sqlTreat, [resLog.ID, proCod, item.ref, item.name, item.location, statusPT, item.divergenceReason, user_name], (e) => e ? rej(e) : res()));
                    }
                }
